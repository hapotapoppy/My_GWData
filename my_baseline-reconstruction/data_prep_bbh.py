...
def gen_masses(m_min=1.1,M_max=5.0,mdist='astro',verbose=True):
    """
    ...
    if mdist=='astro':
        ...
        while not flag:
            # 随机生成两个质量，范围在 1.1 到 2.5 之间
            # 使用对数均匀分布
            m12 = np.exp(np.log(new_m_min) + np.random.uniform(0, 1, 2) * (np.log(2.5) - np.log(new_m_min)))
            
            # 约束条件：总质量 < 5.0 且 m1 >= m2
        ...
        m12 = np.zeros(2)
        while not flag:
            # 中子星质量比 q 通常接近 1，这里设为 1 到 2
            q = np.random.uniform(1.0, 2.0)
            # 较小的中子星质量 m2 选在 1.1 到 2.0 之间
            m12[1] = np.random.uniform(1.1, 2.0)
            m12[0] = m12[1] * q
            
            # 约束条件：单个质量不超过 2.5
            flag = True if (np.all(m12 <= 2.5)) and (np.all(m12 >= 1.1)) and (m12[0] >= m12[1]) else False
        eta = m12[0]*m12[1]/(m12[0]+m12[1])**2
        mc = np.sum(m12)*eta**(3.0/5.0)
        return m12, mc, eta
    elif mdist=='metric':
        ...
        eta_min = m_min * (2.5 - m_min) / (m_min + 2.5)**2
        while not flag:
            # My修改
            ...
            M = (new_M_min**(-7.0/3.0) - np.random.uniform(0,1)*(new_M_min**(-7.0/3.0) - new_M_max**(-7.0/3.0)))**(-3.0/7.0)
            eta = (eta_min**(-2.0) - np.random.uniform(0,1)*(eta_min**(-2.0) - 16.0))**(-1.0/2.0)
            ...
            #m12[1] = M - m12[0]
            m12[0] = (0.5*M + M*np.sqrt(0.25-eta)).item()
            m12[1] = (M - m12[0]).item()
            flag = True if (np.sum(m12)<new_M_max) and (np.all(m12>new_m_min)) and (m12[0]>=m12[1]) else False
        mc = np.sum(m12)*eta**(3.0/5.0)
        return m12, mc, eta
