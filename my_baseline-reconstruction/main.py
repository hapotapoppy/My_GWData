...
class MyResNet(nn.Module):
    def __init__(self):
        ...
        # 修改第一层：原始 ResNet 接受 3 通道图片，我们要改为 1 通道输入
        # 输入维度: [Batch, 1, 2, Length]
        self.resnet.conv1 = nn.Conv2d(1, 64, kernel_size=(1, 7), stride=(1, 2), padding=(0, 3), bias=False)
        
        # 修改全连接层：原始 ResNet 输出 1000 类，我们要改为 2 类（噪声/信号）
        num_ftrs = self.resnet.fc.in_features
        self.resnet.fc = nn.Linear(num_ftrs, 2)

    def forward(self, x):
        # x shape: [Batch, 1, 2, Length]
...
def load_model(checkpoint_dir=None):
    ...
    if (checkpoint_dir is not None) and (Path(checkpoint_dir).is_dir()):
        ...
        """
        if len(files) == 0:
            print('文件夹是空的！我们将从第 0 轮重新开始训练。')
            return net, 0, [] 
        
        # 如果有文件，再执行加载逻辑
        checkpoint = torch.load(p / files[0], map_location='cpu')
        net.load_state_dict(checkpoint['model_state_dict'])
...
def train(net, lr, nsample_perepoch, epoch, total_epochs,
    ...
    for epoch in range(epoch, epoch + total_epochs):
        ...
        if (test_l <= min(np.asarray(train_loss_history)[:,2])):
...
if __name__ == "__main__":
    ...
    nsample_perepoch = 1000  # Number of samples per epoch
    dataset_train = DatasetGenerator(snr=20, T=4,nsample_perepoch=nsample_perepoch)  # Training dataset
    dataset_test = DatasetGenerator(snr=20, T=4,nsample_perepoch=nsample_perepoch)  # Test dataset
    ...
    checkpoint_dir = './checkpoints_resnet_bns/'
    ...
    train(net, lr, nsample_perepoch, epoch, total_epochs, dataset_train,data_loader, test_iter, notebook=False)  # Train model